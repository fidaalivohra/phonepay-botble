...
namespace BhadraFoods\PhonePeV2\Providers;

use BhadraFoods\PhonePeV2\Contracts\PhonePePayment as PhonePePaymentContract;
use BhadraFoods\PhonePeV2\Facades\PhonePePayment as PhonePePaymentFacade;
use BhadraFoods\PhonePeV2\Forms\PhonePePaymentMethodForm;
use BhadraFoods\PhonePeV2\PhonePePayment;
...
class PhonePeV2ServiceProvider extends ServiceProvider
{
        ...
        $this->app->bind(PhonePePaymentClient::class, function () {
            return new PhonePePaymentClient(
                StandardCheckoutClient::getInstance(
                    (string) get_payment_setting('client_id', PhonePePaymentFacade::getId()),
                    (int) get_payment_setting('client_version', PhonePePaymentFacade::getId()),
                    (string) get_payment_setting('client_secret', PhonePePaymentFacade::getId()),
                    get_payment_setting('environment', PhonePePaymentFacade::getId(), Env::UAT),
                    (bool) get_payment_setting('should_publish_events', PhonePePaymentFacade::getId(), false),
                )
            );
        });
    ...
    public function boot(): void
    {
        ...
        $this
            ->setNamespace('plugins/phonepe-v2')
            ->loadAndPublishViews()
            ->publishAssets()
        ...
        $this->app->booted(function (): void {
            ...
            add_filter(BASE_FILTER_ENUM_ARRAY, function ($values, $class) {
                if ($class === PaymentMethodEnum::class) {
                    $values['PHONEPE_V2'] = PhonePePaymentFacade::getId();
                }
            ...
            add_filter(PAYMENT_FILTER_ADDITIONAL_PAYMENT_METHODS, function (?string $html, array $data): ?string {
                ...
                PaymentMethods::method(PhonePePaymentFacade::getId(), [
                    'html' => view('plugins/phonepe-v2::payment-method', $data)->render(),
                ]);
        ...
        add_filter(PAYMENT_FILTER_AFTER_POST_CHECKOUT, function (array $data, Request $request) {
            ...
            if (! in_array(strtoupper($currentCurrency->title), PhonePePaymentFacade::supportedCurrencies(), true)) {
        ...
        add_filter(PAYMENT_FILTER_PAYMENT_INFO_DETAIL, function (?string $html, Payment $payment): ?string {
            if ($payment->payment_channel == PhonePePaymentFacade::getId()) {
                $response = $this->app->make(PhonePePaymentClient::class)->getStatus($payment->charge_id, true);

                if (! $response) {
                    return $html;
                }

                $html .= view('plugins/phonepe-v2::detail', compact('response'))->render();
            }
